...
const randomNumber = (max, except) => {
	...
	return except.includes(num) ? randomNumber(max, except) : num;
	//
};
...
const ComposeSalad = (props) => {
	...
	const makeRandom = (e) => {
		...
		const extra2 = randomNumber(extras.length, extra1);
		chooseExtras([
			extras[extra1],
			extras[extra2],
			extras[randomNumber(extras.length, [extra1, extra2])]
		]);
				...
				{options.map((name) => (
					<div key={name} className="form-check form-check-inline">
						<input
							...
							style={{
								borderColor: chosenValue.includes(name)
									? "#0d6efd"
									: "rgba(0,0,0,0.25)",
								backgroundColor: chosenValue.includes(name) ? "#0d6efd" : "#fff"
							}}
							type="checkbox"
							id={name}
							name={name}
